...
const STATUS_OPTIONS: Array<{ value: DeliveryStatusFilter; label: string }> = [
  ...
  { value: "Queued", label: "Dang cho gui" },
  { value: "Delivered", label: "Da gui" },
  { value: "Failed", label: "That bai" },
  { value: "Skipped", label: "Bo qua" },
];
...
export default function NotificationsPage() {
  ...
  const fetchDeliveries = useCallback(
    async (showRefreshState = false) => {
      ...
      try {
        ...
        toast.error(
          getApiErrorMessage(error, "Khong the tai danh sach gui thong bao.")
        );
      } finally {
        setIsLoading(false);
  ...
  const handleRetry = async (delivery: NotificationDelivery) => {
    ...
    try {
      ...
      toast.success("Da dua thong bao ve hang doi gui lai.");
      await fetchDeliveries(true);
    } catch (error: unknown) {
      toast.error(getApiErrorMessage(error, "Khong the retry thong bao."));
    } finally {
      setRetryingId(null);
    }
  };
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-slate-900">
              Theo doi gui thong bao
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Giam sat trang thai gui Email va SMS tu pipeline notification.
              Trang nay tu dong lam moi 15 giay de le tan va admin theo doi
              su co delivery ngay trong ca truc.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card className="border border-amber-100 bg-amber-50/60 p-5 shadow-sm hover:shadow-sm">
          ...
          <p className="mt-2 text-sm text-amber-800">
            Can publisher/worker xu ly tiep.
          </p>
        </Card>
        ...
        <Card className="border border-emerald-100 bg-emerald-50/70 p-5 shadow-sm hover:shadow-sm">
          ...
          <p className="mt-2 text-sm text-emerald-800">
            Da co provider message id hoac da xac nhan gui.
          </p>
        </Card>
        ...
        <Card className="border border-rose-100 bg-rose-50/70 p-5 shadow-sm hover:shadow-sm">
          ...
          <p className="mt-2 text-sm text-rose-800">
            Can kiem tra loi va retry neu phu hop.
          </p>
        </Card>
        ...
        <Card className="border border-slate-200 bg-slate-50/80 p-5 shadow-sm hover:shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Thuong do thieu template hoac khong du du lieu nguoi nhan.
          </p>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden border border-slate-100 p-0 shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 px-6 py-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Tong cong {totalCount} thong bao theo bo loc hien tai.
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-16">
            ...
            <p className="text-sm font-medium text-slate-500">
              Dang tai du lieu notification...
            </p>
          </div>
        ) : deliveries.length === 0 ? (
          <div className="p-16 text-center">
            <p className="text-base font-semibold text-slate-700">
              Khong co delivery nao khop bo loc hien tai.
            </p>
            <p className="mt-2 text-sm text-slate-500">
              Thu doi trang thai loc hoac cho worker tao them du lieu.
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {canRetry(delivery.deliveryStatus) ? (
                        ...
                        >
                          {retryingId === delivery.id ? "Dang gui lai..." : "Gui lai"}
                        </Button>
                      ) : (
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-slate-500">
            Hien thi {startItem}-{endItem} / {totalCount} dong.
          </p>
